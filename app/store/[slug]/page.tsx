...
export default async function StorePage({ params }: Props) {
  ...
  const faqs = [
    ...
    { q: `Does ${store.name} offer free shipping?`, a: `${freeCoupons.length > 0 ? `Yes! We currently have ${freeCoupons.length} free shipping offer(s) for ${store.name}.` : `Check ${store.name}'s website for current shipping policies or look for free delivery deals on this page.`}` },
      ...
      <div className="bg-white border-b border-gray-100">
        <div className="container-main py-6">
          ...
          <div className="flex flex-wrap gap-2 mb-5">
            {[
              { l: 'Total Offers', v: allCoupons.length, hi: false },
              { l: 'Active Coupons', v: activeCoupons.length, hi: true },
              { l: 'Best Discount', v: maxDiscount > 0 ? `${maxDiscount}% OFF` : 'Great Deals', hi: true },
              { l: 'Coupon Codes', v: codeCoupons.length, hi: false },
              { l: 'Total Uses', v: totalUses.toLocaleString(), hi: false },
            ].map(({ l, v, hi }) => (
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-orange-50 border-b border-orange-100">
                    {['Offer', 'Best Discount', 'Valid Until', 'Code'].map(h => (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="flex flex-wrap gap-2">
              {[
                { label: 'All Offers', cnt: activeCoupons.length },
                { label: '🏷️ Codes', cnt: codeCoupons.length },
                { label: '🔥 Deals', cnt: dealCoupons.length },
                { label: '🚚 Free Ship', cnt: freeCoupons.length },
                { label: '✅ Verified', cnt: verifiedCoupons.length },
              ].map(t => (
                <span key={t.label} className="inline-flex items-center gap-1.5 px-4 py-2 rounded-full text-sm font-semibold border bg-white text-gray-600 border-gray-200 hover:border-orange-300 hover:text-orange-600 cursor-pointer transition-all">
                  {t.label}
                  <span className="text-xs bg-gray-100 text-gray-500 px-1.5 py-0.5 rounded-full font-bold">{t.cnt}</span>
                </span>
              ))}
            </div>
              ...
              {activeCoupons.length > 0
                ...
                : (
                  <div className="bg-white rounded-xl border border-gray-100 p-10 text-center">
                    ...
                    <p className="font-semibold text-gray-500">No active coupons right now. Check back soon!</p>
                  </div>
                )}
            </div>
            ...
            <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
              ...
              <ol className="space-y-4">
                {[
                  { n: 1, t: 'Find your coupon', d: `Browse verified ${store.name} codes above. Look for the highest discount or best match for your purchase.` },
                  { n: 2, t: 'Click "Get Code" to reveal', d: `Click the orange button. The full code is revealed and copied to clipboard. You are also redirected to ${store.name}.` },
                  { n: 3, t: `Shop on ${store.name}`, d: 'Add products to your cart. Make sure your order meets any minimum cart value requirement.' },
                  { n: 4, t: 'Apply code at checkout', d: 'Find the promo code field at checkout. Paste your code and click Apply to see your discount instantly.' },
                  { n: 5, t: 'Complete purchase and save!', d: 'Confirm your order and enjoy your savings. The discount is applied before payment.' },
                ].map(item => (
                  <li key={item.n} className="flex items-start gap-4">
                    <div className="w-8 h-8 rounded-full bg-orange-500 text-white text-sm font-bold flex items-center justify-center flex-shrink-0">{item.n}</div>
                    <div>
            ...
            {expiredCoupons.length > 0 && (
              <details className="bg-white rounded-xl border border-gray-100 shadow-sm p-5">
                <summary className="cursor-pointer font-semibold text-gray-500 hover:text-gray-700 flex items-center gap-2 list-none">
                  ...
                  Show {expiredCoupons.length} Expired {store.name} Coupons
          ...
          <div className="space-y-5">
            ...
            <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-5">
              ...
              <div className="space-y-3">
                {[
                  ...
                  { l: 'Total Uses', v: totalUses.toLocaleString(), I: Users },
                ].map(({ l, v, I }) => (
                  ...
                  {sidebarStores.map(s => (
                    <Link key={s.id} href={`/store/${s.slug}`} className="flex items-center gap-3 p-2 rounded-lg hover:bg-orange-50 transition-colors group">
                      <div className="w-10 h-10 rounded-xl border border-gray-100 bg-white overflow-hidden flex items-center justify-center flex-shrink-0">
                        {s.logo
                          ? <Image src={s.logo} alt={s.name} width={40} height={40} className="object-contain p-0.5" />
            ...
            <div className="bg-gray-50 border border-gray-200 rounded-xl p-4">
              <p className="text-xs text-gray-400 leading-relaxed">
                <strong className="text-gray-600">Disclaimer:</strong> Coupon availability may change without notice. Some links may be affiliate links. We verify all codes before publishing but cannot guarantee availability at all times.
              </p>
            </div>
